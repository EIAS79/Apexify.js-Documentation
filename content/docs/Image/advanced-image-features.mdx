...
### Clipping vs box stroke (5.3+)

- **`borderRadius` + `borderPosition` (on the image)** control the **clipped** shape: which corners of the bitmap are rounded.
- **`stroke`** on the image is a **rounded-rectangle outline** around that box. Use **`stroke.borderPosition`** to stroke only certain **edges** (e.g. `'top'`, `'left'`, `'top-left'` for two edges). Use **`stroke.roundedCorners`** to choose which corners use **`stroke.borderRadius`** on the outline. This is separate from **glyph** strokes on text (`strokeText`).
...
|---------|----------|-------------|
| **Shadow** | `shadow` | Color or gradient shadows; `roundedCorners` masks the shadow shape (see `ShadowOptions`) |
| **Stroke** | `stroke` | Box outline styles; `borderPosition` = which **edges**; `roundedCorners` = which corners use stroke radius |
...
| **Border Radius** | `borderRadius` | Round the **image clip** (pixels or `'circular'`) |
| **Border Position (clip)** | `borderPosition` | Which **corners** of the image box use `borderRadius` for clipping |
