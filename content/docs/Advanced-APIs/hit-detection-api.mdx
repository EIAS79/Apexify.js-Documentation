...
Regions of type `path` use **`buildPath2DFromCommands`** with the **same command vocabulary** as `createPath2D()` (rounded rect, star, arrow, etc.), so hit targets match what you draw.
