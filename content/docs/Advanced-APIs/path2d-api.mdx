...
Path commands are implemented in one place (`pathCmd.ts`: `PathCommand`, `appendPathCommands`, `buildPath2DFromCommands`) so **Path2D drawing and hit-testing stay in sync**. Stroke and fill each apply `globalAlpha` **before** `stroke()` / `fill()`, and fill/stroke opacity default to **1** when omitted.
