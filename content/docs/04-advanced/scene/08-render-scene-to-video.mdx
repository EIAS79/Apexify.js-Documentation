...
const scene = {
  ...
  layers: [
    { type: "text", texts: [{ text: "Intro", x: 120, y: 420, fontSize: 96, color: "#f8fafc" }] },
  ],
};
...
await painter.renderSceneToVideoFrames(scene, {
  ...
  sceneRender: { maxSurfaceDepth: 24 },
});
```
...
- [Video overview](/docs#00-video-overview) — **`createVideo`** one-op router
- [Video pipeline (advanced)](/docs#01-video-pipeline) — trim, splice, captions, SFX after scene export
- [`createAudio` (advanced)](/docs#01-create-audio) — procedural SFX to mux with **`mixAudio`** or pipeline **`.audio()`**
- [Video (advanced) hub](/docs#00-video-advanced-hub)
