...
**Paths:** Local video paths can be absolute or relative to the current working directory; absolute paths are not prefixed with `cwd` twice. **HTTP(S) sources** are downloaded to a temporary file under the frames output directory before processing (they are not written as raw URL strings).
...
The same **`createVideo()`** router used for trimming, merging, and **encoding from frames** also exposes **single-frame extraction** via the **`extractFrame`** option. Open **[Comprehensive Video Processing](/docs#video-processing)** and use the on-page outline: **Create video from frames** (frame → MP4/WebM) and **2. Extract Single Frame** (video → PNG buffer).

<CodeSwitcher>
<ts>
```typescript
// One frame → PNG buffer + dimensions (nested extractFrame options)
const canvas = await painter.createVideo({
  source: './video.mp4',
  extractFrame: {
    time: 5.5,               // Time in seconds (or use frame: n)
