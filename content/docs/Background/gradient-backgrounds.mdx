...
Gradients are defined using the `gradientBg` property with either a `linear` or `radial` type. Each gradient requires an array of color stops (`{ stop, color }`) that define the transition. Combine `gradientBg` (or `colorBg` / `customBg`) with **`bgLayers`** if you need extra stacked overlays — see [Layered backgrounds](./advanced-background-features#layered-backgrounds-bglayers).
