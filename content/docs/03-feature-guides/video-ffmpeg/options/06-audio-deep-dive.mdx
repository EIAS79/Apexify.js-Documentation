# Video audio operations (FFmpeg) {#06-audio-deep-dive}

Mux, strip, duck, and normalize **existing** audio on video via **`createVideo`**. **`mixAudio`** accepts path/URL/buffer overlays — including WAV **`Buffer`s** from **`painter.createAudio`**.

**Procedural SFX (generate WAV in code):** [Advanced → `createAudio`](/docs#01-create-audio) · [Audio hub](/docs#00-audio-advanced-hub). **Editor stacks:** [Video pipeline](/docs#01-video-pipeline).
