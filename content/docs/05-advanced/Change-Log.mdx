...
The showcase site depends on **`apexify.js`** `5.3.20` (see root `package.json`). Entries below mirror the repository **`CHANGELOG.md`** through **[5.3.20]** and earlier releases.
...
- **`mixAudio`**: mix **multiple** external clips (path, URL, or **`Buffer`**) at chosen **timeline starts**, with trim (**`duration`** / **`sourceStart`**), per-clip **`volume`** (linear gain), optional **`keepOriginalAudio`**, **`originalVolume`**, and per-track **`speed`** / **`pitchSemitones`** (overlay clips and optional **`originalSpeed`** / **`originalPitchSemitones`** on the source audio).
