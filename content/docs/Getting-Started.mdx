...
- **Charts** — `options.appearance` uses the same background model as canvas (`customBg`, `bgLayers`, patterns, noise); comparison charts merge shared `appearance` into each panel.
