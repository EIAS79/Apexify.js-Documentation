...
- **Curved / arc text (`textOnCurve`)** — circular bend with `sweepAngle`, `radius`, `up`; see [Curved text & arcs](/docs#text-on-paths)
...
| **Arc / bend** | `textOnCurve` | Circular arc layout (`sweepAngle`, `radius`, `up`) — see [curved text guide](/docs#text-on-paths) |
