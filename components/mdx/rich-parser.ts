...
export type RichMdxSegment =
  ...
  | { kind: 'component'; name: Doc3RegisteredComponentName; props: Record<string, unknown>; body?: string };
...
function parseJsonValue(source: string): unknown {
  ...
  try {
    ...
    throw new Error(`[doc3-mdx] component expression must be valid JSON: ${value.slice(0, 80)}`);
  }
}
  ...
  while (cursor < source.length) {
    ...
    if (quote === '"' || quote === "'") {
      ...
      while (cursor < source.length && source[cursor] !== quote) {
        if (source[cursor] === '\\' && source[cursor + 1] === quote) {
          value += quote;
          cursor += 2;
        } else {
          value += source[cursor];
    ...
    if (quote === '{') {
      ...
      props[name] = parseJsonValue(source.slice(start, cursor - 1));
      continue;
    }
    ...
    cursor += bare.length;
    props[name] = parseJsonValue(bare);
  }
  return props;
}

export function parseRichMdxSegments(content: string): RichMdxSegment[] {
  const names = DOC3_REGISTERED_COMPONENTS.join('|');
  const paired = new RegExp(`<(${names})\\b([^>]*)>([\\s\\S]*?)<\\/\\1>`, 'g');
  const selfClosing = new RegExp(`<(${names})\\b([^>]*)\\/>`, 'g');
  const markers: Array<{ index: number; end: number; name: Doc3RegisteredComponentName; attrs: string; body?: string }> = [];
  ...
  for (const regex of [paired, selfClosing]) {
    ...
    while ((match = regex.exec(content)) !== null) {
      markers.push({
        ...
        name: match[1] as Doc3RegisteredComponentName,
        attrs: match[2] ?? '',
        body: regex === paired ? match[3] : undefined,
      });
    }
  }
  ...
  if (filtered.length === 0) return [{ kind: 'markdown', content }];

  const segments: RichMdxSegment[] = [];
  let cursor = 0;
  for (const marker of filtered) {
    if (marker.index > cursor) segments.push({ kind: 'markdown', content: content.slice(cursor, marker.index) });
    segments.push({
      kind: 'component',
      name: marker.name,
      props: parseRichMdxAttributes(marker.attrs),
