...
function RichComponent({ name, props, body }: { name: string; props: UnknownProps; body?: string }) {
  const children = body ? <MarkdownFragment content={body} /> : null;
  switch (name) {
...
export function RouteDocsMarkdown({ content }: { content: string }) {
  ...
  return <>{segments.map((segment, index) => segment.kind === 'markdown' ? <MarkdownFragment key={`markdown-${index}`} content={segment.content} /> : <RichComponent key={`${segment.name}-${index}`} name={segment.name} props={segment.props} body={segment.body} />)}</>;
}
