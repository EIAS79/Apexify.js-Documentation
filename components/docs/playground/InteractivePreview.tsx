...
export function InteractivePreview({
  ...
  return (
    ...
    >
      {truthLabel ? (
        <div
          className="shrink-0 px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.16em]"
      ...
      <InteractiveErrorBoundary label="Preview">
        <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
          {status === 'loading' || status === 'resetting' ? (
            <div
              role="status"
              className="grid min-h-[180px] flex-1 place-items-center p-6 text-sm"
            ...
            <div
              className="grid min-h-[180px] flex-1 place-items-center p-6 text-center text-sm"
              style={{ color: 'var(--text-tertiary)' }}
            >
              {status === 'unsupported'
                ? 'Preview is not available for this runtime.'
                : status === 'error'
                  ? 'Preview failed. See diagnostics for details.'
