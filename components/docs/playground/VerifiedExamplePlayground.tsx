...
export function VerifiedExamplePlayground({
  ...
  const diagnostics = useMemo(
    () => [{
      ...
      message: 'Preview is DOC-5 verified output. Local source edits do not execute or replace verification evidence.',
      source: sourceHash,
      help: 'Use Studio only when a legitimate execution adapter is available.',
    }],
    [sourceHash],
  );
  ...
  const copyShareState = async () => {
    try {
      await navigator.clipboard.writeText(serializeInteractiveSession(session));
      setCopied(true);
  ...
  return (
    <section className="my-5 grid gap-3" data-doc8-representative-playground="verified-example">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.14em]" style={{ color: 'var(--text-tertiary)' }}>
            Interactive documentation · Node example
          </p>
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            data-doc8-action="copy-share-state"
            onClick={() => void copyShareState()}
            className="min-h-11 rounded-lg px-3 text-sm font-semibold"
            style={{ border: '1px solid var(--border-default)', background: 'var(--bg-raised)' }}
          >
            {copied ? 'State copied' : 'Copy local state'}
          </button>
        </div>
      </div>
        ...
        <InteractiveWorkspace
          ...
          preview={
            <InteractiveErrorBoundary label="Preview">
              <InteractivePreview
                status={previewUrl ? 'ready' : 'unsupported'}
                label={`${title} preview`}
                provenance="verified"
              >
                {previewUrl ? (
