...
export function StudioFileTabs({
  ...
  useEffect(() => {
    requestAnimationFrame(() => {
      const activeTab = railRef.current?.querySelector<HTMLElement>('[data-studio-tab-active="true"]');
      activeTab?.scrollIntoView({ block: 'nearest', inline: 'nearest', behavior: 'auto' });
    });
  }, [activeId, buffers.length]);
  ...
  return (
    <div
      className="studio-file-tabs-shell flex shrink-0 min-w-0 items-stretch"
      style={{
    ...
    >
      <div
        ref={railRef}
        className="studio-file-tabs flex min-w-0 flex-1 items-end gap-1 overflow-x-auto overflow-y-hidden px-2 pt-1.5 sm:px-3"
        aria-label="Scrollable open snippet tabs"
      >
        {buffers.map((b) => {
          ...
          return (
            <div
              key={b.id}
              data-studio-tab
              data-studio-tab-active={isActive ? 'true' : undefined}
      ...
      <div className="studio-file-tabs-actions flex shrink-0 items-end px-2 pt-1.5">
        <button
          type="button"
          onClick={onNew}
          aria-label="New snippet"
          className="inline-flex shrink-0 items-center gap-1 rounded-md px-2 py-1.5 text-[11px] font-semibold transition-colors focus-visible:ring-2 focus-visible:ring-[var(--ring)]"
          style={{
