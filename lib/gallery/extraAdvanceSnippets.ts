 ...
 * PNGs: `npm run gallery:build` via `writeGalleryStaticOutputs.ts` (keep configs aligned).
 */
...
export const extraAdvanceGalleryItems: AdvanceGalleryCard[] = [
  ...
  },
  {
    ...
    category: 'advance',
    description:
      '**`createChart(\'horizontalBar\')`** with **`labelColor` / `valueColor`** on each row plus **`axes.*.tickColor`** and **`labels.barLabelDefaults`** / **`valueLabelDefaults`** so category names, numerics, and values stay legible on dark panels.\n\n**Takeaway:** Use explicit light ink whenever **`appearance.backgroundColor`** is deep navy—defaults alone can resolve to near-black.',
