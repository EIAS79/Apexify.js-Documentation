...
async function main() {
  ...
  return painter.createChart('bar', data, {
    ...
    labels: { title: { text: 'Bar waterfall · legend top-left', fontSize: 20, color: '#f8fafc' } },
    legend: {
      show: true,
      position: 'top-left',
...
async function main() {
  ...
  return painter.createChart('horizontalBar', data, {
    ...
    labels: { title: { text: 'H-bar stacked · legend top-left', fontSize: 20, color: '#f8fafc' } },
    legend: {
      show: true,
      position: 'top-left',
      entries: [
...
async function main() {
  ...
  return painter.createComparisonChart({
    ...
    appearance,
    generalTitle: {
      text: 'Comparison · pie legend top-right · bar legend top-left',
      fontSize: 22,
      color: '#f8fafc',
    },
    chart1: {
...
export const chartsShowcaseGalleryItems: AdvanceGalleryCard[] = [
  ...
  },
  {
    ...
    category: 'advance',
    description:
      '**`painter.createComboChart`** — **`barsType: \'grouped\'`** with **`values`** segments, plus **`lines`** on **`yAxis: \'primary\'`** vs **`\'secondary\'`** (dual Y). **`legend`** + **`grid`** + **`barStyle.groupSpacing`**.\n\n**Takeaway:** Bars + trends in one canvas; axis tinting follows series when colors omitted.',
  ...
  },
  {
    ...
    category: 'advance',
    description:
      '**`createComparisonChart`** — pie **`legends.standard`** (**`top-right`**) beside **`bar`** legend (**`top-left`**); shared **`appearance`** and **`generalTitle`**.\n\n**Takeaway:** One wide export, **`apexify.js`** API only.',
