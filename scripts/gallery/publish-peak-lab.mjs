...
const sharedChunks = chunkCode(sharedCode, 58);
...
for (let n = 1; n <= 26; n++) {
  ...
  items.push({
    ...
    recipeId: id,
    title: meta.title,
    description: `${meta.description} Generated by Apexify Peak Lab recipe ${id}; preview and source come from the same canonical recipe. The inspector paginates shared helpers and recipe source; embedded font binaries remain in the canonical full file.`,
...
export const peakLabSharedSetupPages = ${JSON.stringify(sharedChunks)};
...
export const peakLabGalleryItems: CuratedGalleryCard[] = ${JSON.stringify(items, null, 2).replace(/"codePages": \[/g, '"codePages": [...setupPages,')};
`;
