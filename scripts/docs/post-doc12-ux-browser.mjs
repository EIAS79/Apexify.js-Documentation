...
try {
  ...
  if (!recoveredHeadline.includes('Build visual systems') || !recoveredHeadline.includes('from code.')) fail.push(`homepage: current hero headline missing (${recoveredHeadline})`);
  ...
  if (afterWorkbench.studio) {
    ...
    const studioButton = await docs.page.$('[data-post-doc12-action="open-in-studio"]');
    if (studioButton) {
      await Promise.all([
