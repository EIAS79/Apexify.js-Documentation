...
const richRenderer=fs.readFileSync(path.join(root,'components/docs/route/RichDocsSegment.tsx'),'utf8');
...
if(!renderer.includes("import('./RichDocsSegment')")) fail('routed renderer does not lazy-load the rich renderer');
for(const name of ['ExecutableExample','CodePreview']) if(!richRenderer.includes(`case '${name}'`)) fail(`${name} is not wired into the routed rich renderer`);
...
console.log(`[doc5-verify] PASS — ${expectedIds.length} verified packed-package examples feed manifest/docs/API/Gallery/detail routes through the lazy rich renderer`);
