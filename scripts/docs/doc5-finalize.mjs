...
const docsManifest = JSON.parse(fs.readFileSync(path.join(root, 'generated', 'docs-doc1', 'docs-manifest.json'), 'utf8'));
const failures = [];
...
if (intentionalRouteExpansion && normalizedBuildPercent > 35) failures.push(`route-normalized build wall regression ${normalizedBuildPercent}% exceeds 35%`);
...
const evidence = {
  ...
  thresholds: {
    buildWallPercentMaxWhenRouteCountStable: 35,
    normalizedBuildWallPercentMaxDuringIntentionalRouteExpansion: 35,
