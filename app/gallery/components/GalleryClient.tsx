...
const FILTER_ORDER_PRIMARY: Exclude<FilterCategory, 'all'>[] = [
  'background',
  'images',
  'charts',
  'text',
...
/** Second row — centered under primary lanes */
const FILTER_ORDER_SECONDARY: Exclude<FilterCategory, 'all'>[] = ['mix', 'advance'];
...
/** Inactive filter chips — lift, glow, gradient wash on hover */
const FILTER_CHIP_HOVER_INACTIVE =
  'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-black focus-visible:ring-white/35 motion-safe:transition-all motion-safe:duration-300 motion-safe:ease-[cubic-bezier(0.34,1.56,0.64,1)] motion-safe:hover:-translate-y-1 motion-safe:hover:scale-[1.035] motion-safe:active:scale-[0.98] motion-safe:active:translate-y-0 hover:border-white/22 hover:bg-white/[0.09] hover:shadow-[0_20px_56px_-24px_rgba(165,180,252,0.22)]';

const FILTER_CHIP_ICON_MOTION =
  'motion-safe:transition-transform motion-safe:duration-300 motion-safe:ease-out group-hover:scale-110 motion-safe:group-hover:-rotate-6';
          ...
          <div className="flex flex-col gap-2 sm:gap-3">
            <div className="flex flex-wrap gap-2 sm:gap-3 items-center justify-center sm:justify-start">
              <button
                ...
                className={`group relative shrink-0 overflow-hidden rounded-2xl border px-4 py-2.5 text-sm font-semibold ${
                  selectedCategory === 'all'
                    ? 'border-white/20 bg-white/10 text-white shadow-[0_0_24px_rgba(99,102,241,0.28)] motion-safe:transition-all motion-safe:duration-300 motion-safe:hover:-translate-y-0.5 motion-safe:hover:scale-[1.03] motion-safe:hover:shadow-[0_16px_48px_-20px_rgba(129,140,248,0.42)] motion-safe:active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-black focus-visible:ring-white/35'
                    : `border-transparent bg-white/[0.04] text-slate-400 hover:text-white ${FILTER_CHIP_HOVER_INACTIVE}`
                }`}
              >
                {selectedCategory !== 'all' && (
                  <span
                    aria-hidden
                    className="pointer-events-none absolute inset-0 bg-gradient-to-r from-violet-600/50 to-indigo-500/40 opacity-0 motion-safe:transition-opacity motion-safe:duration-300 group-hover:opacity-100"
                  />
                )}
                <span className="relative z-[1] flex items-center gap-2">
                  <Squares2X2Icon
                    className={`h-4 w-4 ${selectedCategory === 'all' ? 'motion-safe:transition-transform motion-safe:duration-300 group-hover:scale-105' : FILTER_CHIP_ICON_MOTION}`}
                  />
                  All
                  <span
                    className={`rounded-full px-2 py-0.5 text-[11px] font-bold motion-safe:transition-colors motion-safe:duration-300 ${
                      selectedCategory === 'all'
                        ? 'bg-white/15 text-white group-hover:bg-white/22'
                        : 'bg-white/10 text-slate-300 group-hover:bg-white/18 group-hover:text-white'
                    }`}
                  >
                    {galleryItems.length}
                  </span>
              ...
              {FILTER_ORDER_PRIMARY.map((key) => {
                ...
                return (
                  <button
                    ...
                    className={`group relative shrink-0 overflow-hidden rounded-2xl border px-3.5 sm:px-4 py-2.5 text-sm font-semibold ${
                      isActive
                        ? `border-transparent bg-gradient-to-r ${config.gradient} text-white shadow-lg motion-safe:transition-all motion-safe:duration-300 motion-safe:hover:-translate-y-0.5 motion-safe:hover:scale-[1.03] motion-safe:hover:shadow-xl motion-safe:hover:brightness-[1.06] motion-safe:active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-black focus-visible:ring-white/40`
                        : `border-transparent bg-white/[0.04] text-slate-400 hover:text-white ${FILTER_CHIP_HOVER_INACTIVE}`
                    }`}
                  >
                    {!isActive && (
                      <span
                        aria-hidden
                        className={`pointer-events-none absolute inset-0 bg-gradient-to-r opacity-0 motion-safe:transition-opacity motion-safe:duration-300 group-hover:opacity-[0.2] ${config.gradient}`}
                      />
                    )}
                    <span className="relative z-[1] flex items-center gap-2">
                      <IconComponent
                        className={`h-4 w-4 motion-safe:transition-transform motion-safe:duration-300 motion-safe:ease-out ${isActive ? `text-white ${FILTER_CHIP_ICON_MOTION}` : `${config.color} ${FILTER_CHIP_ICON_MOTION}`}`}
                      />
                      ...
                      <span
                        className={`rounded-full px-2 py-0.5 text-[11px] font-bold motion-safe:transition-colors motion-safe:duration-300 ${
                          isActive
                            ...
                            : 'bg-white/10 text-slate-400 group-hover:bg-white/16 group-hover:text-slate-200'
                        }`}
                      >
                        {count}
                      </span>
          ...
          {filteredItems.length === 0 && (
            <div className="text-center py-20 rounded-3xl border border-dashed border-white/10 bg-white/[0.02]">
              ...
              <p className="text-slate-500 max-w-md mx-auto text-sm">
                Add cards under <code className="text-slate-400">lib/gallery/&lt;category&gt;/</code> (see <code className="text-slate-400">background/</code>,{' '}
                <code className="text-slate-400">charts/</code>, <code className="text-slate-400">advance/</code>, …) and map{' '}
                <code className="text-slate-400">discoverCategories</code> in{' '}
...
function GalleryIosCodeWindow({
  ...
  const langLabel = codeLang === 'ts' ? 'TypeScript' : 'JavaScript';
  ...
  return (
    <div
      className="flex flex-col flex-1 min-h-0 min-w-0 h-full max-h-full rounded-lg overflow-hidden border border-slate-600/80 bg-slate-950 shadow-lg"
      style={{ fontFamily: galleryMono }}
    >
      <div className="flex-shrink-0 flex items-center gap-2 sm:gap-3 px-3 py-2 bg-slate-900 border-b border-slate-700">
        <CodeBracketIcon className="h-4 w-4 text-slate-400 shrink-0" aria-hidden />
        <span className="text-xs font-semibold text-slate-200 truncate min-w-0">{fileLabel}</span>
        <span className="text-[10px] font-medium text-slate-500 shrink-0 uppercase tracking-wide">{langLabel}</span>
        <span className="flex-1 min-w-2" />
        {(hasTs || hasJs) && hasTs && hasJs && (
          <div
            className="flex rounded-md bg-slate-800 p-0.5 shrink-0 border border-slate-700"
            role="tablist"
          ...
          >
            <button
              ...
              className={`px-2 py-1 rounded text-[11px] font-semibold transition-colors ${
                codeLang === 'ts' ? 'bg-slate-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              TS
            </button>
            <button
              ...
              className={`px-2 py-1 rounded text-[11px] font-semibold transition-colors ${
                codeLang === 'js' ? 'bg-slate-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              JS
            </button>
        ...
        <button
          ...
          className="shrink-0 flex h-8 px-2.5 items-center justify-center rounded-md bg-slate-800 text-slate-200 hover:bg-slate-700 border border-slate-600 text-xs font-medium"
        >
          {copied ? <CheckIcon className="h-4 w-4 text-emerald-400" /> : <ClipboardDocumentIcon className="h-4 w-4" />}
        </button>
      </div>
...
function ZoomableStillPreview({ src, alt }: { src: string; alt: string }) {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 min-w-0 gap-2">
      ...
      <div
        ref={scrollRef}
        className={`flex-1 min-h-[180px] overflow-auto rounded-xl bg-black/25 ring-1 ring-white/[0.06] touch-pan-x touch-pan-y p-4 sm:p-6 ${
      ...
      >
        {/* Sized to image + min viewport so scrollWidth/scrollHeight include all edges (flex center alone often clips scroll extents). */}
        <div
        ...
        >
          ...
          <img
            ...
            className={`rounded-lg shadow-2xl select-none ${natural ? 'block max-w-none shrink-0 cursor-[inherit]' : 'object-contain'}`}
          />
        </div>
      </div>
    </div>
...
function GalleryModal({ item, onClose }: { item: GalleryItem; onClose: () => void }) {
  ...
  const codeShellClass = [
    ...
    // Mobile split: fixed minimum panel height + outer modal scroll; lg: share row with internal scroll
    showCode &&
      layoutMode === 'split' &&
      'w-full shrink-0 min-h-[min(52vh,420px)] lg:min-h-0 lg:flex-1 lg:w-1/2 lg:flex-none lg:max-w-[50%] lg:shrink-0',
    showCode && layoutMode === 'code' && 'flex-1 w-full min-h-[min(42vh,340px)] lg:min-h-0 lg:flex-1',
  ]
    .filter(Boolean)
    .join(' ');
  ...
  const mediaShellClass = [
    ...
    showMedia &&
      layoutMode === 'split' &&
      'w-full shrink-0 min-h-0 pb-[max(0.75rem,env(safe-area-inset-bottom))] lg:flex-1 lg:min-h-0 lg:w-1/2 lg:flex-none lg:max-w-[50%] lg:shrink-0 lg:pb-0',
    ...
    <>
      ...
      >
        <div className="pointer-events-auto flex flex-col flex-1 min-h-0 max-h-[100dvh] sm:max-h-[calc(100dvh-1.5rem)] md:max-h-[calc(100dvh-3rem)] w-full max-w-7xl mx-auto bg-slate-950/98 backdrop-blur-xl border-0 sm:border-2 border-slate-800/50 rounded-none sm:rounded-2xl lg:rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row flex-1 min-h-0 overflow-y-auto overflow-x-hidden lg:overflow-hidden overscroll-contain touch-pan-y [-webkit-overflow-scrolling:touch]">
            <div className={codeShellClass}>
              <div className="flex flex-1 min-h-0 min-w-0 h-full flex-col overflow-hidden p-3 sm:p-4 lg:p-5">
                {hasCode ? (
                  <>
                    {hasCode && modalMediaKind === 'video' && (
                      <div className="mb-3 rounded-xl border border-amber-500/25 bg-amber-950/35 px-3 py-2 text-[11px] leading-snug text-amber-100/90">
